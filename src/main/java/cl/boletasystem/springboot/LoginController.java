...
import cl.boletasystem.springboot.service.ILoginService;
import cl.boletasystem.springboot.service.IResetTokenService;
import cl.boletasystem.springboot.util.EmailUtil;
import cl.boletasystem.springboot.util.Mail;
	...
	public ResponseEntity<Integer> enviarCorreo(@RequestBody String correo) {
		...
		try {
			...
			if (us != null && us.getIdUsuario() > 0) {
				...
				Mail mail = new Mail();
				mail.setFrom("info@hype-tech.net");
				mail.setTo(us.getUsername());
				mail.setSubject("RESTABLECER CONTRASEÑA - BOLETASYSTEM");
